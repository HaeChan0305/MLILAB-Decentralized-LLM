...
def split_train_data_non_iid(jsonl_path, dataset_name, major_labels, major_ratio=0.8):
    train = []
    with jsonlines.open(jsonl_path) as file:
        for line in file:
            train.append(line)
    ...
    dataset_1 = []
    dataset_2 = []
    for label, dataset in dataset_per_labels.items():
        if label in major_labels:    
            dataset_1 += dataset[:int(len(dataset) * major_ratio)]
            dataset_2 += dataset[int(len(dataset) * major_ratio):]
        else:
            dataset_1 += dataset[int(len(dataset) * major_ratio):]
            dataset_2 += dataset[:int(len(dataset) * major_ratio)]
    ...
    with open(jsonl_path.replace("train.jsonl", "train_1_noniid.jsonl") , "w") as file: 
        for i in dataset_1: file.write(json.dumps(i) + "\n")
        
    with open(jsonl_path.replace("train.jsonl", "train_2_noniid.jsonl") , "w") as file: 
        for i in dataset_2: file.write(json.dumps(i) + "\n")
...
if __name__=='__main__':
    dataset_name = "r8"
    # split_train_data_non_iid(f"./data/{dataset_name}/{dataset_name}_train.jsonl", dataset_name, ["Earnings and Earnings Forecasts", "Grain", "Shipping", "Interest Rates"], major_ratio=0.8)
    
    print("==== original ====")
    check_class_imbalance_jsonl(f"./data/{dataset_name}/{dataset_name}_train.jsonl", dataset_name)
    print()
    
    print("==== train_1 ====")
    check_class_imbalance_jsonl(f"./data/{dataset_name}/{dataset_name}_train_1_noniid.jsonl", dataset_name)
    print()

    print("==== train_2 ====")
    check_class_imbalance_jsonl(f"./data/{dataset_name}/{dataset_name}_train_2_noniid.jsonl", dataset_name)
    print()